...
for root, directories, filenames in os.walk('.'):
    for name in filenames:
        if not name.endswith('.py') or re.search(
                r'(venv|\.git)/', os.path.join(root, name)):
            continue
    ...
    for key in not_in_yml:
        ...
        with open(strings_path, 'a') as file:
            if file.write(to_write):
                synced = False
                print(
                    f'[green]    New key "{key}" written into {os.path.basename(file.name)}[/]')

    not_in_py = [key for key in obj.keys(
    ) if key not in all_keys and not key.isupper()]
    for key in not_in_py:
        synced = False
        print(
            f'[yellow]    The key "{key}" is not being used in any script[/]')

    if synced:
        print(
            f'[green]    The file "{os.path.basename(strings_path)}" is already synced[/]')
