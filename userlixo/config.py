        ...
        for name, info in items.items():
            if info['notation'] in inactive:
                try:
                    ...
                    print(
                        f'The plugin {plugin_type}/{name} thrown an error: {e}')
                    continue
...
def filter_sudoers(flt, c, u):
    if not u.from_user:
        return
    user = u.from_user
    return user.id in sudoers or (
        user.username and user.username.lower() in sudoers)


def filter_su_cmd(command, prefixes=None, *args, **kwargs):
    prefixes = ''.join(prefixes) if type(
        prefixes) == list else prefixes or os.getenv('PREFIXES') or '.'
...
# I don't use os.getenv('KEY', fallback) because the fallback wil only be
# used if the key doesn't exist. I want to use the fallback also when the
# key exists but it's invalid
user = Client(
