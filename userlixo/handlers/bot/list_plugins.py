...
async def on_list_plugins_u(c, u):
    ...
    text = lang.list_plugins_select_type(
        user=(
            '@' +
            user.me.username if user.me.username else user.me.first_name),
        bot=bot.me.username)
    lines = [[(lang.user_plugins, 'user_plugins 0'),
...
@Client.on_callback_query(filters.sudoers &
                          filters.regex(r'^(?P<type>user|bot)_plugins (?P<page>\d+)'))
async def on_list_plugins_type(c, u):
    ...
    # if the message is /plugins (sent to bot) or it's a callback query
    # 'plugins'
